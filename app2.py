...
df2 = pd.read_excel('infoClass_twitter.xls')

df_symptoms = pd.read_excel('info_most_common.xlsx')
...
for x in range(len(df2['Tweet'])):
    s ='<br>'.join(re.findall('.{1,50}', df2['Tweet'][x]))
    tweet.append(s)
...
df2['Time'] = pd.to_datetime(df2['Time']).dt.strftime('%H:%M')
...
min1 = min(df2['Date'].astype(str))
max1 = max(df2['Date'].astype(str))
...
sidebar = html.Div(
    [
        ...
        dbc.Nav(
           ...
           html.Label(['Select Secondary Keyword'],style={'font-weight':'bold'}),
           dcc.Dropdown(
                       id="dd2",
                       options=dd2_options,
                       multi=True,
